    ...
    <section class="mb-12 max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-6">
            ...
            @foreach ($categories as $category)
                <a href="{{ $category['url'] }}" target="_blank" class="flex flex-col items-center space-y-2 p-3 bg-white rounded-lg shadow hover:shadow-lg transition">
                    <img src="{{ $category['img'] }}" alt="{{ $category['name'] }}" class="h-16 w-16 object-contain" />
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            @foreach ($gallery as $item)
                <div class="bg-white rounded-lg shadow p-2">
                    <img src="{{ $item['img'] }}" alt="{{ $item['alt'] }}" class="w-full h-48 object-cover rounded" />
                    <p class="mt-2 text-center text-sm font-medium">{{ $item['alt'] }}</p>
                </div>
            @endforeach
        </div>
    </section>
